...
set_python_version() {
	...
	if [[ "$PYTHON_VERSION" = "2" ]]; then
		...
		GET_PIP="py${PYTHON_VERSION}-pip"
		GET_NUMPY="py${PYTHON_VERSION}-numpy"
	fi
}
#####################################################################################################################################################
...
#####################################################################################################################################################
check_dependencies() {
	REQUIRED_PKGS=("bash" "bash-completion" "build-base" "curl" "pkgconf" "autoconf" "automake" "libtool" "git" "perl" "python${PYTHON_VERSION}" "python${PYTHON_VERSION}-dev" "${GET_PIP}" "${GET_NUMPY}" "linux-headers" "libffi-dev" "openssl-dev" "zlib-dev" "jpeg-dev")
	#
